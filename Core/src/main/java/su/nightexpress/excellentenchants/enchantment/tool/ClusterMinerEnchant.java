...
public class ClusterMinerEnchant extends GameEnchantment implements MiningEnchant {
    ...
    public ClusterMinerEnchant(@NotNull EnchantsPlugin plugin, @NotNull EnchantManager manager, @NotNull Path file, @NotNull EnchantContext context) {
        super(plugin, manager, file, context);
    }

    @Override
    protected void loadAdditional(@NotNull FileConfig config) {
        this.disableOnCrouch = ConfigValue.create("ClusterMiner.Disable_On_Crouch",
            true,
            "Sets whether or not enchantment will have no effect when crouching."
        ).read(config);

        this.blocksLimit = Modifier.load(config, "ClusterMiner.Block_Limit",
            Modifier.addictive(4).perLevel(1).capacity(16),
            "Max. possible amount of blocks to be mined at the same time.");

        this.affectedBlocks = ConfigValue.forSet("ClusterMiner.Block_List",
